...
## Optional: Groq for more natural replies

`backend/services/llm_service.py` (new) can rephrase the bot's reply
text via Groq's free API so it sounds less canned. It's a thin, fail-soft
layer on top of the existing deterministic pipeline, **not** a
replacement for it:

- Intent classification, quick-reply buttons, and cards (order status /
  refund timeline) stay exactly as deterministic as before — the LLM
  never decides routing or data, only rephrases the sentence.
- If `GROQ_API_KEY` is unset, or the request fails for any reason
  (no key, network error, rate limit, timeout), it silently falls back
  to the canned reply. Nothing breaks either way.

To enable it: grab a free key at https://console.groq.com/keys, then in
`backend/.env` set:

```
GROQ_API_KEY=your-key-here
GROQ_MODEL=llama-3.3-70b-versatile   # default, can leave as-is
```

No code changes needed — `response_service.build_response()` picks it
up automatically once the key is present.
